...
void OrderController::AddOrder(const std_msgs::String &str)
{
    ...
    // blue Blue_boxes_count red Red_boxes_count
    try
    {
    ...
    // TODO : choose the minimum path for this order
//        _map[std::move(order)] = ;
// check the graph

    Print(order->GetName() + "|" +
                order->GetGoalDispatchName() +
                " was added to map[" + std::to_string(_map.size()) + "]");
//    }
}
...
//    _mapCond.wait(uLck, [this] { return !_map.empty(); });
